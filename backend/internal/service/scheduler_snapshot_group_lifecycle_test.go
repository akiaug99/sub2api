...
func schedulerCanonicalLoadCountForTest() int {
	count := len(schedulerSnapshotPlatforms())
...
func TestSchedulerGroupLifecycleInactiveThenActiveAuthoritativelyReopens(t *testing.T) {
	...
	require.Len(t, cache.tokens(), schedulerCanonicalBucketCountForTest())
	require.Equal(t, schedulerCanonicalLoadCountForTest(), accounts.callCount())
...
func TestSchedulerGroupLifecycleEpochPreventsABA(t *testing.T) {
	...
	require.Len(t, firstActiveTokens, schedulerCanonicalBucketCountForTest())
	...
	require.Len(t, allTokens, 2*schedulerCanonicalBucketCountForTest())
