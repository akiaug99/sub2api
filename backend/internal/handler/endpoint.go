...
func DeriveUpstreamEndpoint(inbound, rawRequestPath, platform string) string {
	...
	if service.IsOpenAICompatiblePlatform(platform) {
		switch inbound {
		case EndpointEmbeddings, EndpointImagesGenerations, EndpointImagesEdits:
			return inbound
		case EndpointAlphaSearch, EndpointVideosGenerations, EndpointVideosEdits, EndpointVideosExtensions, EndpointVideos:
